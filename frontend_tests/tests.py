...
class SimpleTest(TestCase):
    ...
    @unittest.skip('Skipping until tests can be fixed')
...
#         pudb.set_trace()
        self.select_assignment('decusability',
                               title='StarCellBio Usability Test',
                               description='$DISPLAY_ASSIGNMENT_INSTRUCTIONS$')
        self.select_assignment('microscopy_usability',
                               title='Microscopy Usability Test',
                               description='$DISPLAY_ASSIGNMENT_INSTRUCTIONS$')
        self.select_assignment('decusability',
                               title='StarCellBio Usability Test',
                               description='$DISPLAY_ASSIGNMENT_INSTRUCTIONS$')
        self.open_assignment('decusability',
                             title='StarCellBio Usability Test',
                             description='$DISPLAY_ASSIGNMENT_INSTRUCTIONS$')
        ...
        self.set_experiment_design_values(experiment_title,
                                          experiment_hypo,
                                          experiment_obj)
...
#         self.assert_experiments(['Test Experiment'])
        ...
        self.assert_on_experiment_design_page()
        self.assert_experiment_design_values(experiment_title,
                                             experiment_hypo,
                                             experiment_obj)
...
#         self.select_new_set_up()
        ...
        self.navigate_via('NEW WESTERN BLOT')
        self.assert_on_western_blot_page()
        self.assert_western_blot_tabs('W. B. 3',
                                      ['W. B. 1', 'W. B. 2', 'ADD\n| +'])
        ...
        # The drag and drop command in selenium does not work on the Mac OSX,
        # only the linux
        print 'old order'
        print old_order
        ...
        self.navigate_via_button(
            'scb_s_facs_tools_instructions_followup_toggle')
        time.sleep(5)
        ...
        self.select_assignment('microscopy_usability',
                               title='Microscopy Usability Test',
                               description='$DISPLAY_ASSIGNMENT_INSTRUCTIONS$')
        self.open_assignment('microscopy_usability',
                             title='Microscopy Usability Test',
                             description='$DISPLAY_ASSIGNMENT_INSTRUCTIONS$')
        self.assert_on_experiment_design_page()
        experiment_title = 'Test Experiment'
        ...
        experiment_obj = 'Sample objective ABC'
        self.set_experiment_design_values(experiment_title,
                                          experiment_hypo, experiment_obj)
        self.navigate_via('ASSIGNMENTS')
        self.assert_on_assignments_page()
        self.select_assignment('microscopy_usability',
                               title='Microscopy Usability Test',
        ...
        self.assert_on_experiment_setup_page()
#         self.select_new_set_up()
#         button = self.find_by_class_name('scb_s_experiment_setup_table_element')  # NOQA
#         button.click()
        time.sleep(3)
    ...
    def read_list(self):
        ...
        elements = self.driver.find_elements_by_class_name(
            'scb_s_western_blot_choose_samples_list_item')
        if (elements == []):
                elements = self.driver.find_elements_by_class_name(
                    'scb_s_western_blot_choose_samples_list')
    ...
    def assert_experiments(self, experiment_list):
        web_experiment_list = self.driver.find_elements_by_class_name(
            'scb_f_open_assignment_experiment')
        self.assertEqual(experiment_list.__len__(),
                         web_experiment_list.__len__())
    ...
    def assert_samples(self, samples_list):
        web_sample_rows = self.driver.find_elements_by_class_name(
            'scb_s_experiment_setup_table_row')
        web_sample_list = [y for y in set([x.get_attribute('cell_treatment_id')
                                           for x in web_sample_rows])]
        self.assertEqual(samples_list.__len__(), web_sample_list.__len__())

    def add_sample(self, sample):
        add_button = self.find_by_class_name(
            'scb_f_experiment_setup_action_open_add_samples_dialog')
        add_button.click()
        self.find_by_class_name('scb_mit706s16_dialog')
        self.select_option(sample['cell_line'], 'cell_line',
                           'scb_f_experiment_setup_dialog_checkbox')
#         self.select_option(sample['treatment_id'], 'treatment_id', 'scb_f_experiment_setup_dialog_checkbox')  # NOQA
#         self.select_option(sample['collection_id'], 'value','scb_s_experiment_setup_dialog_collection_select_option')  # NOQA
        add_dialog_button = self.find_by_class_name(
            'scb_mit706s16_inner_dialog_add')
        add_dialog_button.click()

    def remove_sample(self, sample):
        remove_button = self.find_by_class_name(
            'scb_f_experiment_setup_remove_sample')
        remove_button.click()

    def assert_western_blots(self, western_blot_titles):
        web_wb_rows = self.driver.find_elements_by_class_name(
            'scb_f_open_western_blot')
    ...
    def assert_western_blot_tabs(self, western_blot_active,
                                 western_blot_titles):
        web_active_western_blot = self.find_by_class_name(
            'scb_s_western_blot_selected')
        self.assertEqual(western_blot_active,
                         (web_active_western_blot.text if
                          len(web_active_western_blot.text) > 0 else
                          web_active_western_blot.get_attribute('value')))
        web_wb_rows = self.driver.find_elements_by_class_name(
            'scb_f_open_western_blot')
        self.assertEqual(western_blot_titles.__len__(), web_wb_rows.__len__())
        web_wb_list = [x.text for x in web_wb_rows]
    ...
    def remove_western_blot(self):
        web_active_western_blot_remove_button = self.find_by_class_name(
    ...
    def select_wb_antibody(self, sample):
        time.sleep(1)
        self.select_option(sample['primary_antibody'], 'model_id',
                           'scb_f_wb_anti_body_select_primary_option')
        time.sleep(1)
        self.select_option(sample['secondary_antibody'], 'model_id',
                           'scb_f_wb_anti_body_select_secondary_option')
        pass

    def select_option(self, value, attribute, css_class):
        web_options = self.driver.find_elements_by_class_name(css_class)
        web_filtered = [x for x in web_options if
                        x.get_attribute(attribute) == value]
        self.assertGreater(web_filtered.__len__(), 0)
        option = web_filtered[0]
        option.click()

    def select_gel_type(self):
        e_class = self.find_by_class_name(
            'scb_s_western_blot_choose_gel_type_input')
        e_class.click()
    ...
    def select_lysates(self):
        cbs = self.driver.find_elements_by_css_selector(
            '.scb_f_western_blot_sample_active')
        for i in range(0, cbs.__len__()):
            ...
            select = self.driver.find_elements_by_css_selector(
                '.scb_f_western_blot_select_lysate_type')
            options = select[i].find_elements_by_tag_name('option')
            option = [x for x in options if x.get_attribute('value') == 'whole']  # NOQA
            if(option.__len__() > 0):
                option[0].click()

    def select_lysates_facs(self):
        cbs = self.driver.find_elements_by_css_selector(
            '.scb_f_facs_sample_active')
        for i in range(0, cbs.__len__()):
            checkboxes = self.driver.find_elements_by_css_selector(
                '.scb_f_facs_sample_active')
            cb = checkboxes[i]
    ...
    def test_sortable(self):
        script = ("$($('.scb_s_western_blot_choose_samples_list_item')[0])"
                  ".simulateDragSortable({ move: 1 });")
        success = self.driver.execute_script(script)  # NOQA
#       with open(settings.rel('../html_app/js/jquery-1.7.2.js')) as jquery_js_2:  # NOQA
#               jquery = jquery_js_2.read()  # NOQA
#       self.driver.execute_script(jquery)  # NOQA
#       with open(settings.rel('jquery.simulate.drag-sortable.js')) as jquery_js_3:  # NOQA
#               jquery_drag = jquery_js_3.read()  # NOQA
#       self.driver.execute_script(jquery_drag)  # NOQA
        # Need this library to be injected beyond the jquery to make sure you don't get errors for the other selectors  # NOQA
#       with open(settings.rel('../html_app/js/jquery.ba-bbq.min.js')) as jquery_js_4:  # NOQA
#               jquery_bbq = jquery_js_4.read()  # NOQA
#       self.driver.execute_script(jquery_bbq)  # NOQA
