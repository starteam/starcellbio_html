...
scb_microscopy.display_slide_types = function(opt_data, opt_sb) {
  ...
  if (soy.$$getMapKeys(opt_data.kinds).length == 1) {
    var kList841 = soy.$$getMapKeys(opt_data.kinds);
    var kListLen841 = kList841.length;
    for (var kIndex841 = 0; kIndex841 < kListLen841; kIndex841++) {
      var kData841 = kList841[kIndex841];
      output.append('<span class="scb_f_microscopy_select_slide_type ', (opt_data.lane.is_sample_enabled) ? '' : 'scb_s_microscopy_dim_conditions', '" cell_treatment_id=\'', soy.$$escapeHtml(opt_data.cell_treatment.id), '\' microscopy_id=\'', soy.$$escapeHtml(opt_data.microscopy.id), '\' assignment_id="', soy.$$escapeHtml(opt_data.assignment.id), '" experiment_id="', soy.$$escapeHtml(opt_data.experiment.id), '" value="', soy.$$escapeHtml(kData841), '" lane_kind="', soy.$$escapeHtml(opt_data.lane.kind), '" lane_id="', (opt_data.lane.kind == 'existing') ? soy.$$escapeHtml(opt_data.lane.lane.id) : '', '">', soy.$$escapeHtml(opt_data.kinds[kData841].name), '</span>');
    }
  } else {
    ...
    if (opt_data.lane.kind == 'existing') {
      if (opt_data.lane.cell_treatment.cell_line == opt_data.assignment.template.model.microscopy.valid[0] && opt_data.lane.cell_treatment.treatment_list.list[0].drug_list.list[0].drug_id == opt_data.assignment.template.model.microscopy.valid[1]) {
        var kList892 = soy.$$getMapKeys(opt_data.assignment.template.slide_parser[opt_data.lane.cell_treatment.treatment_list.list[0].collection_id]);
        var kListLen892 = kList892.length;
        for (var kIndex892 = 0; kIndex892 < kListLen892; kIndex892++) {
          var kData892 = kList892[kIndex892];
          output.append('<option value=\'', soy.$$escapeHtml(kData892), '\'', (opt_data.lane.lane.kind == kData892) ? 'selected="selected"' : '', '>', soy.$$escapeHtml(opt_data.kinds[kData892].name), '</option>');
        }
      } else {
        var kList904 = soy.$$getMapKeys(opt_data.assignment.template.slide_parser['default']);
        var kListLen904 = kList904.length;
        for (var kIndex904 = 0; kIndex904 < kListLen904; kIndex904++) {
          var kData904 = kList904[kIndex904];
          output.append('<option value=\'', soy.$$escapeHtml(kData904), '\'', (opt_data.lane.lane.kind == kData904) ? 'selected="selected"' : '', '>', soy.$$escapeHtml(opt_data.kinds[kData904].name), '</option>');
        }
      }
    } else {
      ...
      if (opt_data.lane.cell_treatment.cell_line == opt_data.assignment.template.model.microscopy.valid[0] && opt_data.lane.cell_treatment.treatment_list.list[0].drug_list.list[0].drug_id == opt_data.assignment.template.model.microscopy.valid[1]) {
        var kList921 = soy.$$getMapKeys(opt_data.assignment.template.slide_parser[opt_data.lane.cell_treatment.treatment_list.list[0].collection_id]);
        var kListLen921 = kList921.length;
        for (var kIndex921 = 0; kIndex921 < kListLen921; kIndex921++) {
          var kData921 = kList921[kIndex921];
          output.append('<option value=\'', soy.$$escapeHtml(kData921), '\'>', soy.$$escapeHtml(opt_data.kinds[kData921].name), '</option>');
        }
      } else {
        var kList929 = soy.$$getMapKeys(opt_data.assignment.template.slide_parser['default']);
        var kListLen929 = kList929.length;
        for (var kIndex929 = 0; kIndex929 < kListLen929; kIndex929++) {
          var kData929 = kList929[kIndex929];
          output.append('<option value=\'', soy.$$escapeHtml(kData929), '\'>', soy.$$escapeHtml(opt_data.kinds[kData929].name), '</option>');
        }
      }
    }
