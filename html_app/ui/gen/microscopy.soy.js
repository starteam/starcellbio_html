  ...
  if (soy.$$getMapKeys(opt_data.kinds).length == 1) {
    ...
    if (opt_data.lane.kind == 'existing') {
      if (opt_data.lane.cell_treatment.cell_line == opt_data.assignment.template.model.microscopy.valid[0] && opt_data.lane.cell_treatment.treatment_list.list[0].drug_list.list[0].drug_id == opt_data.assignment.template.model.microscopy.valid[1]) {
        var kList698 = soy.$$getMapKeys(opt_data.assignment.template.slide_parser[opt_data.lane.cell_treatment.treatment_list.list[0].collection_id]);
        var kListLen698 = kList698.length;
        for (var kIndex698 = 0; kIndex698 < kListLen698; kIndex698++) {
          var kData698 = kList698[kIndex698];
          output.append('<option value=\'', soy.$$escapeHtml(kData698), '\'', (opt_data.lane.lane.kind == kData698) ? 'selected="selected"' : '', '>', soy.$$escapeHtml(opt_data.kinds[kData698].name), '</option>');
        }
      } else {
        var kList710 = soy.$$getMapKeys(opt_data.assignment.template.slide_parser['default']);
        var kListLen710 = kList710.length;
        for (var kIndex710 = 0; kIndex710 < kListLen710; kIndex710++) {
          var kData710 = kList710[kIndex710];
          output.append('<option value=\'', soy.$$escapeHtml(kData710), '\'', (opt_data.lane.lane.kind == kData710) ? 'selected="selected"' : '', '>', soy.$$escapeHtml(opt_data.kinds[kData710].name), '</option>');
        }
      }
    } else {
      ...
      if (opt_data.lane.cell_treatment.cell_line == opt_data.assignment.template.model.microscopy.valid[0] && opt_data.lane.cell_treatment.treatment_list.list[0].drug_list.list[0].drug_id == opt_data.assignment.template.model.microscopy.valid[1]) {
        var kList727 = soy.$$getMapKeys(opt_data.assignment.template.slide_parser[opt_data.lane.cell_treatment.treatment_list.list[0].collection_id]);
        var kListLen727 = kList727.length;
        for (var kIndex727 = 0; kIndex727 < kListLen727; kIndex727++) {
          var kData727 = kList727[kIndex727];
          output.append('<option value=\'', soy.$$escapeHtml(kData727), '\'>', soy.$$escapeHtml(opt_data.kinds[kData727].name), '</option>');
        }
      } else {
        var kList735 = soy.$$getMapKeys(opt_data.assignment.template.slide_parser['default']);
        var kListLen735 = kList735.length;
        for (var kIndex735 = 0; kIndex735 < kListLen735; kIndex735++) {
          var kData735 = kList735[kIndex735];
          output.append('<option value=\'', soy.$$escapeHtml(kData735), '\'>', soy.$$escapeHtml(opt_data.kinds[kData735].name), '</option>');
        }
      }
    }
