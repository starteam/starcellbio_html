...
scb_western_blot.display_tabs = function(opt_data, opt_sb) {
  ...
  for (var rIndex57 = 0; rIndex57 < rListLen57; rIndex57++) {
    ...
    output.append('<tr><td><input type="checkbox" class="scb_f_western_blot_sample_active" western_blot_id=\'', soy.$$escapeHtml(opt_data.western_blot.id), '\' assignment_id=\'', soy.$$escapeHtml(opt_data.assignment.id), '\' experiment_id=\'', soy.$$escapeHtml(opt_data.experiment.id), '\'></td><td>', (rData57.display_sample) ? soy.$$escapeHtml(rData57.cell_treatment.name) : '', '</td><td>');
    scb_western_blot.display_lysate_types({assignment: opt_data.assignment, experiment: opt_data.experiment, western_blot: opt_data.western_blot, lane: rData57}, output);
    output.append('</td><td>', (rData57.kind == 'existing') ? '<button>X</button>' : '', '</td></tr>');
  }
...
scb_western_blot.display_lysate_types = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  output.append('<select class=\'scb_f_western_blot_lysate_type\' western_blot_id=\'', soy.$$escapeHtml(opt_data.western_blot.id), '\' assignment_id=\'', soy.$$escapeHtml(opt_data.assignment.id), '\' experiment_id=\'', soy.$$escapeHtml(opt_data.experiment.id), '\' lane_kind=\'', soy.$$escapeHtml(opt_data.lane.kind), '\'><option selected="selected" disabled="disabled">Pick Lysate Type</option><option>Whole Cell</option><option>Cyto</option><option>Proto</option></select>');
  return opt_sb ? '' : output.toString();
};
