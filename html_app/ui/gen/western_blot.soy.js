...
scb_western_blot.display_tabs_selector = function(opt_data, opt_sb) {
  ...
  var wbList65 = opt_data.experiment.western_blot_list.list;
  var wbListLen65 = wbList65.length;
  for (var wbIndex65 = 0; wbIndex65 < wbListLen65; wbIndex65++) {
    var wbData65 = wbList65[wbIndex65];
    output.append((opt_data.western_blot.id == wbData65.id) ? '<span class=\'scb_s_western_blot_active\'><span class=\'scb_s_western_blot_selected\' western_blot_id=\'' + soy.$$escapeHtml(wbData65.id) + '\' assignment_id=\'' + soy.$$escapeHtml(opt_data.assignment.id) + '\' experiment_id=\'' + soy.$$escapeHtml(opt_data.experiment.id) + '\' contenteditable="true">' + soy.$$escapeHtml(wbData65.name) + '</span><button class=\'scb_f_western_blot_remove\' western_blot_id=\'' + soy.$$escapeHtml(wbData65.id) + '\' assignment_id=\'' + soy.$$escapeHtml(opt_data.assignment.id) + '\' experiment_id=\'' + soy.$$escapeHtml(opt_data.experiment.id) + '\'>X</button></span>' : '<a class=\'scb_f_open_western_blot\' href=\'#view=western_blot&experiment_id=' + soy.$$escapeHtml(opt_data.experiment.id) + '&assignment_id=' + soy.$$escapeHtml(opt_data.assignment.id) + '&western_blot_id=' + soy.$$escapeHtml(wbData65.id) + '\' western_blot_id=\'' + soy.$$escapeHtml(wbData65.id) + '\'>' + soy.$$escapeHtml(wbData65.name) + '</a>');
  }
  output.append('</div>');
  return opt_sb ? '' : output.toString();
};
...
scb_western_blot.sample_prep = function(opt_data, opt_sb) {
  ...
  var rList101 = opt_data.rows;
  var rListLen101 = rList101.length;
  for (var rIndex101 = 0; rIndex101 < rListLen101; rIndex101++) {
    var rData101 = rList101[rIndex101];
    output.append('<tr><td>', (rData101.display_sample) ? '<input type="checkbox" class="scb_f_western_blot_sample_active" western_blot_id=\'' + soy.$$escapeHtml(opt_data.western_blot.id) + '\' assignment_id=\'' + soy.$$escapeHtml(opt_data.assignment.id) + '\' experiment_id=\'' + soy.$$escapeHtml(opt_data.experiment.id) + '\' cell_treatment_id=\'' + soy.$$escapeHtml(rData101.cell_treatment.id) + '\'' + ((rData101.is_sample_enabled) ? 'checked="checked"' : '') + '>' : '', '</td><td>', (rData101.display_sample) ? soy.$$escapeHtml(rData101.cell_treatment.name) : '', '</td><td>');
    scb_western_blot.display_lysate_types({assignment: opt_data.assignment, experiment: opt_data.experiment, western_blot: opt_data.western_blot, cell_treatment: rData101.cell_treatment, lane: rData101}, output);
    output.append('</td><td>', (rData101.kind == 'existing') ? '<button class="scb_f_western_blot_sample_remove" western_blot_id=\'' + soy.$$escapeHtml(opt_data.western_blot.id) + '\' assignment_id=\'' + soy.$$escapeHtml(opt_data.assignment.id) + '\' experiment_id=\'' + soy.$$escapeHtml(opt_data.experiment.id) + '\' lane_id=\'' + soy.$$escapeHtml(rData101.lane.id) + '\'' + ((rData101.is_sample_enabled) ? '' : 'disabled="disabled"') + '>X</button>' : '', '</td></tr>');
  }
...
scb_western_blot.prepare_gel = function(opt_data, opt_sb) {
  ...
  for (var rIndex194 = 0; rIndex194 < rListLen194; rIndex194++) {
    ...
    output.append((rData194.display_sample) ? '<li>' + soy.$$escapeHtml(rData194.cell_treatment.name) + ' -' + soy.$$escapeHtml(rData194.lane.kind) + '</li>' : '');
  }
