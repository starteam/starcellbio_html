  ...
  for (var rIndex252 = 0; rIndex252 < rListLen252; rIndex252++) {
    ...
    if (rData252.lane && soy.$$getMapKeys(opt_data.kinds).length == 1) {
      ...
      if (rData252.kind == 'existing') {
        var kList372 = soy.$$getMapKeys(opt_data.kinds);
        var kListLen372 = kList372.length;
        for (var kIndex372 = 0; kIndex372 < kListLen372; kIndex372++) {
          var kData372 = kList372[kIndex372];
          var cList373 = soy.$$getMapKeys(opt_data.kinds[kData372].conditions);
          var cListLen373 = cList373.length;
          for (var cIndex373 = 0; cIndex373 < cListLen373; cIndex373++) {
            var cData373 = cList373[cIndex373];
            output.append('<option value=\'', soy.$$escapeHtml(cData373), '\' role=\'option\' ', (rData252.lane.conditions == cData373) ? 'selected="selected"' : '', '>', soy.$$escapeHtml(opt_data.kinds[kData372].conditions[cData373].name), '</option>');
          }
          output.append((soy.$$getMapKeys(opt_data.kinds[kData372].conditions).length >= 1) ? (! rData252.lane.conditions) ? '<option  role=\'option\'selected="selected" disabled="disabled" value=\'\'>Select Condition</option>' : '' : '');
        }
      }
    }
  ...
  if (soy.$$getMapKeys(opt_data.kinds).length == 1) {
    ...
    if (opt_data.lane.kind == 'existing') {
      ...
      for (var kIndex506 = 0; kIndex506 < kListLen506; kIndex506++) {
        ...
        output.append('<option value=\'', soy.$$escapeHtml(kData506), '\' role=\'option\'', (opt_data.lane.lane.kind == kData506) ? 'selected="selected"' : '', '>', soy.$$escapeHtml(opt_data.kinds[kData506].name), '</option>');
      }
    } else {
      ...
      var kList521 = soy.$$getMapKeys(opt_data.kinds);
      var kListLen521 = kList521.length;
      for (var kIndex521 = 0; kIndex521 < kListLen521; kIndex521++) {
        var kData521 = kList521[kIndex521];
        output.append('<option role=\'option\' value=\'', soy.$$escapeHtml(kData521), '\'>', soy.$$escapeHtml(opt_data.kinds[kData521].name), '</option>');
      }
    }
...
scb_facs.analyze = function(opt_data, opt_sb) {
  ...
  var rList566 = opt_data.rows;
  var rListLen566 = rList566.length;
  for (var rIndex566 = 0; rIndex566 < rListLen566; rIndex566++) {
    var rData566 = rList566[rIndex566];
    output.append((rData566.is_valid) ? (rData566.index == 0) ? '<li role=\'listitem\' facs_id=\'' + soy.$$escapeHtml(opt_data.facs.id) + '\' assignment_id="' + soy.$$escapeHtml(opt_data.assignment.id) + '" experiment_id="' + soy.$$escapeHtml(opt_data.experiment.id) + '" facs_lane_id=\'' + soy.$$escapeHtml(rData566.lane.id) + '\' lane_kind = \'' + soy.$$escapeHtml(rData566.lane.kind) + '\' conditions = \'' + soy.$$escapeHtml(rData566.lane.conditions) + '\' title=\'' + soy.$$escapeHtml(rData566.display_text) + '\'' + ((opt_data.facs.selected_lane && rData566.cell_treatment.id == opt_data.facs.selected_lane.cell_treatment_id) ? 'class=\'scb_s_facs_sample_selected\'' : '') + '>&nbsp;&nbsp;' + soy.$$escapeHtml(rData566.display_text) + '</li>' : '' : '');
  }
  ...
  if (opt_data.facs.samples_finished) {
    var rList618 = opt_data.rows;
    var rListLen618 = rList618.length;
    for (var rIndex618 = 0; rIndex618 < rListLen618; rIndex618++) {
      var rData618 = rList618[rIndex618];
      output.append((rData618.is_valid && rData618.lane.id == opt_data.facs.selected_lane.id) ? '<span class=\'scb_s_western_blot_gel_active scb_s_western_blot_gel_tab \' role=\'tab\'><div class=\'scb_s_facs_gel_tab_selected\'>' + soy.$$escapeHtml(opt_data.facs.selected_lane.kinds[opt_data.facs.selected_lane.kind].conditions[opt_data.facs.selected_lane.conditions].name) + '</div></span>' : (rData618.is_valid && rData618.cell_treatment.id == opt_data.facs.selected_lane.cell_treatment_id) ? '<div class=\'scb_s_western_blot_gel_tab\' facs_id=\'' + soy.$$escapeHtml(opt_data.facs.id) + '\' assignment_id=\'' + soy.$$escapeHtml(opt_data.assignment.id) + '\' role=\'tab\' experiment_id=\'' + soy.$$escapeHtml(opt_data.experiment.id) + '\' facs_lane_id=\'' + soy.$$escapeHtml(rData618.lane.id) + '\'>' + soy.$$escapeHtml(rData618.lane.kinds[rData618.lane.kind].conditions[rData618.lane.conditions].name) + '</div>' : '');
    }
  } else {
    ...
    if (opt_data.lane.canvas_metadata_analysis) {
      if (opt_data.lane.canvas_metadata_analysis.ranges) {
        if (opt_data.lane.canvas_metadata_analysis.ranges.length > 0) {
          ...
          var rangeList737 = opt_data.lane.canvas_metadata_analysis.ranges;
          var rangeListLen737 = rangeList737.length;
          for (var rangeIndex737 = 0; rangeIndex737 < rangeListLen737; rangeIndex737++) {
            var rangeData737 = rangeList737[rangeIndex737];
            output.append('<tr role=\'row\'><td class="', (rangeData737.bisector_id == 'b') ? 'scb_s_facs_tools_analyze_bisector_border' : '', '">', soy.$$escapeHtml(rangeData737.display_id), '</td><td class="', (rangeData737.bisector_id == 'b') ? 'scb_s_facs_tools_analyze_bisector_border' : '', '">', soy.$$escapeHtml(rangeData737.bisector_id), '</td><td class="', (rangeData737.bisector_id == 'b') ? 'scb_s_facs_tools_analyze_bisector_border' : '', '">', soy.$$escapeHtml(rangeData737.from), ' - ', soy.$$escapeHtml(rangeData737.to), '</td><td class="', (rangeData737.bisector_id == 'b') ? 'scb_s_facs_tools_analyze_bisector_border' : '', '">', soy.$$escapeHtml(rangeData737.percentage), '</td><td class="', (rangeData737.bisector_id == 'b') ? 'scb_s_facs_tools_analyze_bisector_border' : '', '"><img class=\'scb_f_facs_analyze_remove_point scb_s_facs_analyze_remove_point\' assignment_id=\'', soy.$$escapeHtml(opt_data.assignment.id), '\' experiment_id=\'', soy.$$escapeHtml(opt_data.experiment.id), '\' facs_id=\'', soy.$$escapeHtml(opt_data.facs.id), '\' facs_lane_id=\'', soy.$$escapeHtml(opt_data.lane.id), '\' from=\'', soy.$$escapeHtml(rangeData737.from), '\' to=\'', soy.$$escapeHtml(rangeData737.to), '\' alt="Delete" title="Delete" src="images/setup/scb_remove.png"></td></tr>');
          }
