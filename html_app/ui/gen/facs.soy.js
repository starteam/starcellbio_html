...
scb_facs.display_lysate_types = function(opt_data, opt_sb) {
  ...
  if (opt_data.lane.kind == 'existing') {
    var kList294 = soy.$$getMapKeys(opt_data.kinds);
    var kListLen294 = kList294.length;
    for (var kIndex294 = 0; kIndex294 < kListLen294; kIndex294++) {
      var kData294 = kList294[kIndex294];
      output.append('<option value=\'', soy.$$escapeHtml(kData294), '\'', (opt_data.lane.lane.kind == kData294) ? 'selected="selected"' : '', '>', soy.$$escapeHtml(opt_data.kinds[kData294].name), '</option>');
    }
  } else {
    ...
    var kList309 = soy.$$getMapKeys(opt_data.kinds);
    var kListLen309 = kList309.length;
    for (var kIndex309 = 0; kIndex309 < kListLen309; kIndex309++) {
      var kData309 = kList309[kIndex309];
      output.append('<option value=\'', soy.$$escapeHtml(kData309), '\'>', soy.$$escapeHtml(opt_data.kinds[kData309].name), '</option>');
    }
  }
...
scb_facs.analyze = function(opt_data, opt_sb) {
  ...
  var rList327 = opt_data.rows;
  var rListLen327 = rList327.length;
  for (var rIndex327 = 0; rIndex327 < rListLen327; rIndex327++) {
    var rData327 = rList327[rIndex327];
    output.append((rData327.is_valid) ? '<li facs_id=\'' + soy.$$escapeHtml(opt_data.facs.id) + '\' assignment_id="' + soy.$$escapeHtml(opt_data.assignment.id) + '" experiment_id="' + soy.$$escapeHtml(opt_data.experiment.id) + '" facs_lane_id=\'' + soy.$$escapeHtml(rData327.lane.id) + '\'' + ((opt_data.facs.lane_selected == rData327.lane.id) ? 'class=\'scb_s_facs_sample_selected\'' : '') + '>' + soy.$$escapeHtml(rData327.display_text) + ' - ' + soy.$$escapeHtml(rData327.lane.kinds[rData327.lane.kind].name) + '</li>' : '');
  }
    ...
    if (opt_data.lane.canvas_metadata_analysis) {
      if (opt_data.lane.canvas_metadata_analysis.ranges) {
        if (opt_data.lane.canvas_metadata_analysis.ranges.length > 0) {
          ...
          var rangeList391 = opt_data.lane.canvas_metadata_analysis.ranges;
          var rangeListLen391 = rangeList391.length;
          for (var rangeIndex391 = 0; rangeIndex391 < rangeListLen391; rangeIndex391++) {
            var rangeData391 = rangeList391[rangeIndex391];
            output.append('<tr><td><div style=\'background-color:', soy.$$escapeHtml(rangeData391.color), '; width:12px; height:12px\'></div></td><td>', soy.$$escapeHtml(rangeData391.from), ' - ', soy.$$escapeHtml(rangeData391.to), '</td><td>', soy.$$escapeHtml(rangeData391.percentage), '</td><td><img class=\'scb_f_facs_analyze_remove_point\' assignment_id=\'', soy.$$escapeHtml(opt_data.assignment.id), '\' experiment_id=\'', soy.$$escapeHtml(opt_data.experiment.id), '\' facs_id=\'', soy.$$escapeHtml(opt_data.facs.id), '\' facs_lane_id=\'', soy.$$escapeHtml(opt_data.lane.id), '\' from=\'', soy.$$escapeHtml(rangeData391.from), '\' to=\'', soy.$$escapeHtml(rangeData391.to), '\' alt="Delete" title="Delete" src="images/setup/scb_remove.png"></td></tr>');
          }
