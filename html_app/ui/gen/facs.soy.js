...
scb_facs.display_lysate_types = function(opt_data, opt_sb) {
  ...
  if (soy.$$getMapKeys(opt_data.kinds).length == 1) {
    ...
    if (opt_data.lane.kind == 'existing') {
      var kList476 = soy.$$getMapKeys(opt_data.kinds);
      var kListLen476 = kList476.length;
      for (var kIndex476 = 0; kIndex476 < kListLen476; kIndex476++) {
        var kData476 = kList476[kIndex476];
        output.append('<option value=\'', soy.$$escapeHtml(kData476), '\' role=\'option\'', (opt_data.lane.lane.kind == kData476) ? 'selected="selected"' : '', '>', soy.$$escapeHtml(opt_data.kinds[kData476].name), '</option>');
      }
    } else {
      ...
      for (var kIndex491 = 0; kIndex491 < kListLen491; kIndex491++) {
        ...
        output.append('<option role=\'option\' value=\'', soy.$$escapeHtml(kData491), '\'>', soy.$$escapeHtml(opt_data.kinds[kData491].name), '</option>');
      }
    }
...
scb_facs.analyze = function(opt_data, opt_sb) {
  ...
  var rList536 = opt_data.rows;
  var rListLen536 = rList536.length;
  for (var rIndex536 = 0; rIndex536 < rListLen536; rIndex536++) {
    var rData536 = rList536[rIndex536];
    output.append((rData536.is_valid) ? '<li role=\'listitem\'  facs_id=\'' + soy.$$escapeHtml(opt_data.facs.id) + '\' assignment_id="' + soy.$$escapeHtml(opt_data.assignment.id) + '" experiment_id="' + soy.$$escapeHtml(opt_data.experiment.id) + '" facs_lane_id=\'' + soy.$$escapeHtml(rData536.lane.id) + '\'' + ((opt_data.facs.lane_selected == rData536.lane.id) ? 'class=\'scb_s_facs_sample_selected\'' : '') + '>&nbsp;&nbsp;' + soy.$$escapeHtml(rData536.display_text) + ' - ' + soy.$$escapeHtml(rData536.lane.kinds[rData536.lane.kind].name) + '</li>' : '');
  }
    ...
    if (opt_data.lane.canvas_metadata_analysis) {
      if (opt_data.lane.canvas_metadata_analysis.ranges) {
        if (opt_data.lane.canvas_metadata_analysis.ranges.length > 0) {
          ...
          var rangeList655 = opt_data.lane.canvas_metadata_analysis.ranges;
          var rangeListLen655 = rangeList655.length;
          for (var rangeIndex655 = 0; rangeIndex655 < rangeListLen655; rangeIndex655++) {
            var rangeData655 = rangeList655[rangeIndex655];
            output.append('<tr role=\'row\'><td><div style=\'background-color:', soy.$$escapeHtml(rangeData655.color), '; width:12px; height:12px\'></div></td><td>', soy.$$escapeHtml(rangeData655.from), ' - ', soy.$$escapeHtml(rangeData655.to), '</td><td>', soy.$$escapeHtml(rangeData655.percentage), '</td><td><img class=\'scb_f_facs_analyze_remove_point\' assignment_id=\'', soy.$$escapeHtml(opt_data.assignment.id), '\' experiment_id=\'', soy.$$escapeHtml(opt_data.experiment.id), '\' facs_id=\'', soy.$$escapeHtml(opt_data.facs.id), '\' facs_lane_id=\'', soy.$$escapeHtml(opt_data.lane.id), '\' from=\'', soy.$$escapeHtml(rangeData655.from), '\' to=\'', soy.$$escapeHtml(rangeData655.to), '\' alt="Delete" title="Delete" src="images/setup/scb_remove.png"></td></tr>');
          }
