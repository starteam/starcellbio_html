...
scb_instructor_course_setup.main = function(opt_data, opt_sb) {
  ...
  scb_instructor_course_setup.display_assignment({global_template: opt_data.global_template, assignments: opt_data.assignments, view: 'select_course'}, output);
  output.append('</div>');
...
scb_instructor_course_setup.display_assignment = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  output.append('<div class=\'scb_s_course_setup_description \'><div class=\'scb_s_abstract_title\'>Assignment Editor</div>', (opt_data.view == 'select_course') ? '<div class=\'scb_s_course_setup_title\'>Course Setup</div>' : (opt_data.view == 'create_assignment') ? '' : '', '</div>');
  return opt_sb ? '' : output.toString();
};
